...
export const Results: React.FC<ResultsProps> = ({ userId }) => {
  ...
  useEffect(() => {
    getResuls("blek").then((res) => setBlekResult(res));

    getResuls("edge").then((res) => setEdgeResult(res));

    getResuls("unpossible").then((res) => setUnpossibleResult(res));
  }, []);
    ...
    <div>
      ...
      <Message>
        ...
        <Message.List>
          {blekResult.length > 0 ? (
            <Message.Item>
              Número de niveles completados en Blek (Minuto 10): {blekResult[0]}
              .
            </Message.Item>
          ) : (
            <Message.Item>Todavía no hay resultados para Blek.</Message.Item>
          )}

          {edgeResult.length > 0 ? (
            <Message.Item>
              Número de niveles completados en Edge (Minuto 12): {edgeResult[0]}
              .
            </Message.Item>
          ) : (
            <Message.Item>Todavía no hay resultados para Edge.</Message.Item>
          )}

          {unpossibleResult.length > 0 ? (
            <Message.Item>
              Número de intentos realizados en Unpossible (Minuto 5):{" "}
              {unpossibleResult[0]}.
            </Message.Item>
          ) : (
            <Message.Item>
          ...
          {blekResult.length == 0 &&
            ...
            edgeResult.length == 0 && (
              <Message.Item>
                Parece que todavía no has completado ningún juego. Cuando
                llegues al final de alguno de ellos, podrás encontrar aquí tus
                resultados.
              </Message.Item>
            )}
        </Message.List>
